...
setwd("E:\\OneDrive\\OneDrive - University of Cambridge\\Work\\BWC Hospital\\data_calcs\\ingredient_to_impact_tools")

comp_funcs_00 <- "clark_mod\\0.0_Functions_Estimating_Composition_22January2022.R"

clark_dat_path <- "dat\\clark_data\\Clark_et_al_2022_PNAS_SM\\Data Inputs"


#products_dat <- 'clark_mod\\products_HOMOGENISED.csv'
#categories_dat <- 'dat\\clark_data\\Clark_et_al_2022_PNAS_SM\\foodDB_dat\\categories anonymised.csv'

#products_dat <- 'clark_mod\\products_concat.csv'
#categories_dat <- 'clark_mod\\categories_concat.csv'

output_dir <- paste0("clark_mod", "\\Outputs")
...
#####
###
#products_dat <- paste0(getwd(),"/foodDB_dat/products.csv")
...
for(retail in (retailers)) {
  ...
  #for(z in 1:loops) { ### MOD
    ...
    # Saving data frame
    write.csv(dat.tot,
              paste0(output_dir,"\\FoodDB percent composition by ingredient ",
